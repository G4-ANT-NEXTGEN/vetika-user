<template>
  ...
  <div class="form-section">
    ...
    <p class="description light-dark">Enter the 6-digit code sent to your email or phone</p>
    ...
    <form>
      ...
      <div class="helper-text text-start light-dark">
        Didn't receive code?
...
const otp = computed({
  get: () => authStore.otpForget,
  set: (val) => authStore.otpForget = val
})

onMounted(async () => {
  await nextTick()

  // Fill inputs if otp exists in store
  if (otp.value) {
    const otpArr = otp.value.split("")
    inputs.value.forEach((input, i) => {
      if (input && otpArr[i]) {
        input.value = otpArr[i]
      }
    })
  }
